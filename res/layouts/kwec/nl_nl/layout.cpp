...
class DutchKWeCLayout : public Dyscover::ILayoutProvider {
public:
    const std::vector<KeyTranslationEntry>& GetEntries() const override {
        return g_dutchKWeC;
    }
    const char* GetName() const override { return "dutchkwec"; }
    const char* GetLanguage() const override { return "nl_nl"; }
};

// Static registration
static DutchKWeCLayout s_layout;
static bool s_registered = []() {
    Dyscover::LayoutRegistry::Instance().Register("dutchkwec", &s_layout);
    return true;
}();
